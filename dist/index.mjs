...
//#endregion
//#region src/sso-state.ts
const { get: _getSSOState, set: setSSOState } = defineRequestState(() => void 0);
/**
...
const signInSSOBodySchema = z.object({
	...
	additionalData: z.record(z.string(), z.unknown()).meta({ description: "Additional data to pass through the SSO flow. Available in provisionUser on callback." }).optional(),
...
const signInSSO = (options) => {
	return createAuthEndpoint("/sign-in/sso", {
		...
		if (provider.oidcConfig && body.providerType !== "saml") {
			...
			const state = await generateState(ctx, void 0, options?.redirectURI?.trim() ? {
				ssoProviderId: provider.providerId,
				...ctx.body.additionalData ? { additionalData: ctx.body.additionalData } : {}
			} : ctx.body.additionalData ? { additionalData: ctx.body.additionalData } : false);
		...
		if (provider.samlConfig) {
			...
			const { state: relayState } = await generateRelayState(ctx, void 0, ctx.body.additionalData || false);
...
async function handleOIDCCallback(ctx, options, providerId, stateData) {
	...
	await setSSOState(stateData.additionalData);
	...
	if (options?.provisionUser && linked.isRegister) {
		...
		user = await ctx.context.internalAdapter.findUserById(user.id) ?? user;
	}
...
const callbackSSOSAML = (options) => {
	return createAuthEndpoint("/sso/saml2/callback/:providerId", {
		...
		await setSSOState(relayState?.additionalData);
		let provider = null;
		if (options?.defaultSSO?.length) {
			const matchingDefault = options.defaultSSO.find((defaultProvider) => defaultProvider.providerId === providerId);
			if (matchingDefault) provider = {
		...
		if (options?.provisionUser) {
			await options.provisionUser({
				...
				userInfo,
				provider,
				additionalData: relayState?.additionalData
			});
			user = await ctx.context.internalAdapter.findUserById(user.id) ?? user;
		}
		await assignOrganizationFromProvider(ctx, {
